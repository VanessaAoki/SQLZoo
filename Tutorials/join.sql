...
FROM game JOIN goal ON (game.id = goal.matchid AND (team1 = 'POL' OR team2 = 'POL'))
...
SELECT matchid, mdate, COUNT(teamid) AS 'goals scored'
FROM game JOIN goal ON (game.id = goal.matchid AND (team1 = 'GER' OR team2 = 'GER') AND teamid = 'GER')
GROUP BY matchid, mdate
